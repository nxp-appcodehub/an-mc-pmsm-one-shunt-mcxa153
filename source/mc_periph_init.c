...
void InitClock(void)
{
    ...
    g_sClockSetup.ui32FastPeripheralClock = CLOCK_GetFreq(kCLOCK_FroHf);
...
static void InitPWM(void)
{
    ...
    PWMBase->SM[0].VAL4 = PWM_VAL4_VAL4((uint16_t)(-(g_sClockSetup.ui16M1PwmModulo / 3)));
    PWMBase->SM[1].VAL4 = PWM_VAL4_VAL4((uint16_t)(0));
    PWMBase->SM[2].VAL4 = PWM_VAL4_VAL4((uint16_t)(-(g_sClockSetup.ui16M1PwmModulo / 2)));

    PWMBase->SM[0].VAL5 = PWM_VAL5_VAL5((uint16_t)(-(g_sClockSetup.ui16M1PwmModulo / 6)));
    PWMBase->SM[1].VAL5 = PWM_VAL5_VAL5((uint16_t)(0));
    PWMBase->SM[2].VAL5 = PWM_VAL5_VAL5((uint16_t)(0));

    /* PWM0 module 0 VAL0,VAL4,VAL5 trigger ADC bus current sample */
    PWMBase->SM[0].TCTRL |= PWM_TCTRL_OUT_TRIG_EN(0x31) | PWM_TCTRL_TRGFRQ(1);
    /* PWM0 module 1 VAL1 trigger ADC bus voltage sample */
    PWMBase->SM[1].TCTRL |= PWM_TCTRL_OUT_TRIG_EN(0x2) | PWM_TCTRL_TRGFRQ(1);
    PWMBase->SM[2].TCTRL |= PWM_TCTRL_OUT_TRIG_EN(0x10) | PWM_TCTRL_TRGFRQ(1);
...
static void InitADC(void)
{
    ...
    lpadcCommandConfig.channelNumber = CUR_DCB_CHANNEL_NUMBER;
...
static void InitINPUTMUX(void)
{
    ...
    /* PWM0_SM1_OUT_TRIG1 is selected as trigger input for ADC0 trigger 1 */
    INPUTMUX_AttachSignal(INPUTMUX0, 1U, kINPUTMUX_Pwm0Sm1OutTrig1ToAdc0Trigger);

    /* PWM0_SM0_OUT_TRIG0 is selected as trigger input for AOI_IN0 */
    INPUTMUX_AttachSignal(INPUTMUX0, 0U, kINPUTMUX_Pwm0Sm0OutTrig0ToAoi0Mux);
    /* PWM0_SM0_OUT_TRIG1 is selected as trigger input for AOI_IN1 */
    ...
    aoiEventLogicStruct.PT0AC = kAOI_InputSignal;    /* PWM0_SM0_OUTTRIG0 output */
    ...
    aoiEventLogicStruct.PT1BC = kAOI_InputSignal;    /* PWM0_SM0_OUTTRIG1 output */
...
void ADC_Trig_Attatch(void)
{
    INPUTMUX_AttachSignal(INPUTMUX0, 0U, kINPUTMUX_Aoi0Out0ToAdc0Trigger);//Compressor sampling
}
